...
image::image/doc tech v2/Crediter_Debiter/uml.jpg[width=500]
...
Pour realiser un debit ou un credit, il faut que le compte soit actif et que le montant soit superieur a 0. Si le compte est inactif, on ne peut pas acceder aux operations on a donc pas a gerer l'exception du compte avec un "if". Si le montant est inferieur a 0, on ne peut pas faire l'operation. La condition dont je parle ce trouve dans le fichier OperationsManagementController.java dans la methode doAjouter() a la ligne 237.

image::image/crediterCodeSignificatif.png[]

Comme on peut le voir sur l'image, si le montant est inferieur a 0, on ne peut pas faire l'operation. cela met le style du montant en rouge et remet le focus sur le montant pour pouvoir modifier la somme.
...
image::image/doc tech v2/Prelevement/uml.png[width=400]

*Diagramme de classe :*

image::image/doc tech v2/Prelevement/diagramme de classe.jpg[]
...
* PrelevementManagement.java


Package application.view :

* PrelevementEditorPaneController.java
* PrelevementManagementController.java
